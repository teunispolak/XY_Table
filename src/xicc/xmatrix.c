...
double       detail		/* gamut detail level, 0.0 = def */
	...
	for (i = 0; i < 3; i++) {
		...
		while (co_e < 3) {
            ...
            /* expand the gamut by one point */
			gam->expand(gam, out);

            //printf("~1 i %d, e %d, co_e %d, co %d %d %d, ep %d %d %d, in %f %f %f, out %f %f %f \n",i,e,co_e, co[0],co[1],co[2],ep[0],ep[1],ep[2],in[0],in[1],in[2],out[0],out[1],out[2]);
