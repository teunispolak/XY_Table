...
//  fprintf(stderr,"               P = absolute perceptual, S = absolute saturation\n");
    ...
    fprintf(stderr," -O            Output gamutfile name (without .gam)\n");  // Tepo Jan 2020: Output file name
	fprintf(stderr,"\n");
	exit(1);
}
...
main(int argc, char *argv[]) {
	...
	for(fa = 1;fa < argc;fa++) {
		...
		if (argv[fa][0] == '-')	{	/* Look for any flags */
			...
			/* Search order */
//            else if (argv[fa][1] == 'o' || argv[fa][1] == 'O') {   Tepo Jan 2020 Output file name
                else if (argv[fa][1] == 'o') {
				fa = nfa;
    ...
    //strcpy(out_name, prof_name);    Tepo Jan 2020: output file name
    if (out_name == NULL)
