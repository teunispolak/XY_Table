...
icxViewCond *vc
    ...
    printf(" %s\n",vc->desc);
    if (vc->Ev == vc_dark) {
        printf("  Background to image luminance ratio:  Yb = dark (vc_dark)\n");
    }
    else if (vc->Ev == vc_dim) {
        printf("  Background to image luminance ratio:  Yb = dim (vc_dim)\n");
    }
    else if (vc->Ev == vc_average) {
        printf("  Background to image luminance ratio:  Yb = average (vc_average)\n");
    }
    else if (vc->Ev == vc_cut_sheet) {
        printf("  Background to image luminance ratio:  Yb = transparency (vc_cut_sheet)\n");
    }
    else if (vc->Ev == vc_none) {
        printf("  Background to image luminance ratio:  Yb = auto (vc_none), calculated as La/Lv\n");
    }

    printf("  Background to image luminance ratio:  Yb    = %f\n", vc->Yb);
    printf("  Illuminance:                          E     = %f Lux, calculated as Lv*pi\n",vc->Lv * 3.1415);
    printf("  Image luminance:                      Lv    = %f cd/m^2\n",vc->Lv);
    printf("  Background luminance:                 La    = %f cd/m^2\n",vc->La);
    printf("  Background white point:               Wxyz  = %f %f %f\n",vc->Wxyz[0], vc->Wxyz[1], vc->Wxyz[2]);
    printf("  Flare to image luminance ratio:       Yf    = %f\n",vc->Yf);
    printf("  Glare to image luminance ratio:       Yg    = %f\n",vc->Yg);
    if (vc->Yg > 0.0)
        printf("  Glare white point:                    Gxyz  = %f %f %f\n",vc->Gxyz[0], vc->Gxyz[1], vc->Gxyz[2]);
    printf("  HK effect scaling factor:             hk    = %f\n",vc->hkscale);
}
