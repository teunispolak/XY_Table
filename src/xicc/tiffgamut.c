...
#include "sort.h"
#include "vrml.h"
#include "cam02.h"          /* include ref to printCAM */

#undef NOCAMGAM_CLIP		/* No clip to CAM gamut before CAM lookup */
#undef DEBUG				/* Dump filter cell contents */


#define READ_FROM_FILE       /* tepo Oct 2016 Read hard-coded RGB values from code, not from TIFF file */
...
/*  tepo Oct 2016: wrap pathname in quotes
 
    If a string (e.g. command line argument) contains a forward slash, it contains a pathname
    Then the pathname needs to be wrapped in quotes */

void wrapPath(char* arg, int index)
{
    char *strC;
    char *quote = "\"";
    int len;
    
    len = strlen(arg) + strlen(quote) + strlen(quote);
    strC = (char*)malloc(len + 1);
    
    strncpy(strC,arg,index);            /* add the charcters before the first forward slash (might be zero) */
    strC[index] = '\0';                 /* add the null character */
    strcat(strC,quote);                 /* add the first quote */
    strcat(strC,arg + index);           /* add the characters after the first forward slash */
    strcat(strC,quote);                 /* add the last quote */
...
main(int argc, char *argv[]) {
	...
	for(fa = 1;fa < argc;fa++) {
		...
		if (argv[fa][0] == '-')	{	/* Look for any flags */
			...
			/* Surface Detail */
                ...
                if (na == NULL) usage();
                gamres = atof(na);
                /* tepo Oct 2019: check on range added, similar to iccgamut.c */
                if (gamres < 1.0 || gamres > 50.0)
                    usage();
			}
			/* Filtering */
    ...
    /*  tepo Oct 2019: save command line arguments
     *
     *  if an argument contains a path, it must be wrapped in quotes
     *
    ...
    while(i < argc)                                     /* for each argument */
    {
        ...
        strcpy(str, argv[i]);                           /* copy argument to a string */
        ptr = strchr(str, '/');
        if (ptr)                                        /* if string contains "/" it's a path */
        {
            index = ptr - str;                          /* position of the first "/" in the argument */
            wrapPath(str, index);                       /* then wrap path in quotes */
        }
        printf("%s \n", str);
        i++;
    }
	...
	if (prof_name[0] != '\000') {
        ...
        /* tepo Oct 2019: print the viewing conditions.   */
        if (verb) {
            printf("Input viewing conditions:\n");
            xicc_dump_viewcond(&vc);
            
            printCAM = 1;
            printf("\nInput CIECAM02 values\n");
        }
...
#else   /* not reading from file, but from hard-coded input */
    
    /* well, let's make use of the luo */
    ...
    if (luo != NULL) {
        ...
        for (i=0; i<7; i++){
            for (j=0; j<3; j++)
                 rgb[j] = cc24[i][j] / 255.0;

            printf("RGB (0..255)) = %f %f %f\n", cc24[i][0], cc24[i][1], cc24[i][2]);
            if ((rv = luo->lookup(luo, jab, rgb)) > 1)
                error ("%d, %s",icco->errc,icco->err);
            printf("Scaled  Jab = %f %f %f\n\n",jab[0],jab[1],jab[2]);
        }
    }
#endif  /* end of READ_FROM_FILE yes or no switch. Clean up luo and cam */
...
#ifdef READ_FROM_FILE
    /* When reading from TIFF file, expand gamut if required and create output files */
	if (verb)
