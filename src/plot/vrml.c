...
double vdist
    ...
    } else if (s->ispace == vrml_jab) {     /* Jab, leav 0..100 */
        s->scale = 1.0;
        s->off = 50.0;                      /* z axis offset */
	...
	if (doaxes != 0) {
        ...
        /*  tepo Oct 2019 added Jab, so from axes[3][6] to axes[4][6] */
        ...
        /*  tepo Oct 2019 added Jab, so added ispace == 3 condition */
        if (s->ispace == 3) {
            j = 3;
            if (s->fmt == fmt_vrml)
