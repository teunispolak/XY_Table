...
//				           bpo,
		...
		/* Create B2A clut */
		{
			...
			for (i = 0; i < 2; i++) {
                ...
                // precies verkeerd om; vc hkscale nu hard gezet in set_cam
//                if (v->hkscale >= 0.0)
//                    vc->hkscale = v->hkscale;
                v->hkscale = vc->hkscale;

                /* Nov 2018     Print the viewing conditions.   */
                if (verb) {
                    if (i == 0)
                        printf("Input viewing condition:\n");
                    else if (i == 1)
                        printf("Output viewing condition:\n");
			...
			/* we get the chance to set ICX_CAM_CLIP. */
			{
                ...
                if (verb)
                    printf("Input / source CIECAM02 values\n");
			...
			if (!allintents) {	/* Only B2A0, no intent */
				...
				if (src_xicc || gcompr) {	/* Creating separate perceptual and Saturation tables */
					...
					if (src_xicc != NULL) {
                        ...
                        /* Not so subtle, but the lowest impact
                         *
                         * Set global constant theSource to trigger the source viewing condition variable Fl
                         * to be stored as global constant srcFl in set_view of cam02.c
                         *
                         * srcFl is used in an experimental option in the Jab -> XYZ reverse transform
                         *
                         */
                        theSource = 1;
                    ...
                    if (verb)
                        printf("Output / destination CIECAM02 values\n");
                    
                    /* Set flag for ciecam02 set_view; see above comments for theSource */
                    theDest = 1;
